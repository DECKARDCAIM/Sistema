...
namespace Sistema.Presentacion
{
    public partial class FrmIngreso : Form
    {
        ...
        //IMPLEMENTACION DEL HASHTABLE
        private Hashtable articulosHashtable = new Hashtable();
        ...
        private void CrearTabla()
        {
            ...
            DgvListado.Columns[5].HeaderText = "IMPORTE";
        ...
        private void TxtCodigo_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        this.MensajeError("NO EXISTE ESE ARTICULO CON ESE CODIGO DE BARRAS");
                    }
                    else
                    {
                        int IdArticulo = Convert.ToInt32(Tabla.Rows[0][0]);
                        string Codigo = Convert.ToString(Tabla.Rows[0][1]);
                        string Nombre = Convert.ToString(Tabla.Rows[0][2]);
                        decimal Precio = Convert.ToDecimal(Tabla.Rows[0][3]);

                        // UN IF PARA VERIFICAR SI EL ARTICULO YA EXISTE DENTRO DEL HASHTABLE
                        if (articulosHashtable.ContainsKey(Codigo))
                        {
                            this.MensajeError("EL ARTICULO YA HA SIDO AGREGADO");
                        }
                        else
                        {
                            articulosHashtable.Add(Codigo, Nombre); // SINO EXISTE AGREGARLO AL HASHTABLE DE articulosHastable
                            this.AgregarDetalle(IdArticulo, Codigo, Nombre, Precio); //AGREGANDO AL DETALLE DE COMPRA UNICAMENTE ID, CODIGO, NOMBRE Y PRECIO
                        }
                    }
                }
            }
